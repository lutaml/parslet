...
class Parslet::Optimizers::CutInserter
  ...
  #
  # @param parslet [Parslet::Atoms::Base] The parslet to optimize
  ...
  def optimize_sequence(seq)
    ...
    Parslet::Atoms::Sequence.new(*optimized_parslets)
  end
  ...
  #
  # @param sequence [Parslet::Atoms::Sequence] The sequence to analyze
  # @return [Array<Parslet::Atoms::Base>] Prefix parslets, or nil if none
  ...
  def safe_to_cut?(parslet)
    ...
    return false if first.include?(Parslet::FirstSet::EPSILON)
  ...
  #
  # @param sequence [Parslet::Atoms::Sequence] Original sequence
  # @param prefix_parslets [Array] Parslets forming the deterministic prefix
  # @return [Parslet::Atoms::Base] New sequence with cut inserted
  def build_cut_sequence(sequence, prefix_parslets)
    ...
    prefix = if optimized_prefix.length == 1
      ...
      Parslet::Atoms::Sequence.new(*optimized_prefix)
    end
    ...
    if optimized_remaining.empty?
      ...
      Parslet::Atoms::Sequence.new(prefix.cut, *optimized_remaining)
    end
  end
end
