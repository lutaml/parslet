...
describe "Tree Memoization" do
  let(:context) { Parslet::Atoms::Context.new(nil, interval_cache: true) }

  describe "Repetition with tree memoization" do
    it "caches repeated parsing of same element" do
      parser = Parslet::Atoms::Str.new('a').repeat(1, 3)
      source = Parslet::Source.new('aaa')
    ...
    it "reuses cached prefix for repetitions" do
      parser = Parslet::Atoms::Str.new('x').repeat(2, 5)
      source = Parslet::Source.new('xxxxx')
    ...
    it "handles variable repetitions with .maybe" do
      parser = Parslet::Atoms::Str.new('b').maybe
      source = Parslet::Source.new('b')
    ...
    it "handles empty repetitions" do
      parser = Parslet::Atoms::Str.new('c').repeat(0, 2)
      source = Parslet::Source.new('')
    ...
    it "respects min bound in tree memoization" do
      parser = Parslet::Atoms::Str.new('d').repeat(2, 4)
      source = Parslet::Source.new('d')
    ...
    it "respects max bound in tree memoization" do
      parser = Parslet::Atoms::Str.new('e').repeat(1, 3)
      source = Parslet::Source.new('eeeee')
  ...
  describe "Context tree memoization methods" do
    ...
    it "returns false for use_tree_memoization? when disabled" do
      context_no_tree = Parslet::Atoms::Context.new
  ...
  describe "Integration with complex parsers" do
    it "handles nested repetitions" do
      inner = Parslet::Atoms::Str.new('a')
      outer = inner.repeat(1, 2).repeat(1, 2)
      source = Parslet::Source.new('aaaa')
  ...
  describe "Performance characteristics" do
    it "benefits from caching on repeated parses at same position" do
      parser = Parslet::Atoms::Str.new('m').repeat(3, 5)
      source = Parslet::Source.new('mmmmm')

      # First parse - miss
      result1 = parser.apply(source, context, false)

      # Second parse at same position - should hit cache
      source2 = Parslet::Source.new('mmmmm')
    ...
    it "handles large repetitions efficiently" do
      parser = Parslet::Atoms::Str.new('z').repeat(10, 50)
      input = 'z' * 50
      source = Parslet::Source.new(input)
  ...
  describe "Error handling" do
    it "returns proper errors when min not met" do
      parser = Parslet::Atoms::Str.new('q').repeat(3, 5)
      source = Parslet::Source.new('qq')
    ...
    it "handles unconsumed input errors" do
      parser = Parslet::Atoms::Str.new('r').repeat(1, 2)
      source = Parslet::Source.new('rrr')
  ...
  describe "Backward compatibility" do
    it "works without tree memoization enabled" do
      context_no_tree = Parslet::Atoms::Context.new
      parser = Parslet::Atoms::Str.new('t').repeat(2, 4)
      source = Parslet::Source.new('tttt')
