...
describe "FIRST set computation" do
  ...
  describe "Str atom" do
    it "returns itself as FIRST set" do
      ...
      expect(first.first).to be_a(Parslet::Atoms::Str)
  ...
  describe "Re atom" do
    it "returns itself as FIRST set" do
      ...
      first = atom.first_set
      expect(first.size).to eq(1)
      expect(first.first).to be_a(Parslet::Atoms::Re)
    end
  end
  ...
  describe "Sequence atom" do
    ...
    it "propagates through EPSILON when first element can match empty" do
      ...
      strs = first.select { |x| x.is_a?(Parslet::Atoms::Str) }.map(&:str)
  ...
  describe "Repetition atom" do
    it "includes EPSILON for min=0 (maybe)" do
      atom = str('a').maybe
      first = atom.first_set
      expect(first).to include(Parslet::FirstSet::EPSILON)
      strs = first.select { |x| x.is_a?(Parslet::Atoms::Str) }
    ...
    it "does not include EPSILON for min=1" do
      ...
      first = atom.first_set
      expect(first).not_to include(Parslet::FirstSet::EPSILON)
    end
  ...
  describe "Lookahead atom" do
    it "returns EPSILON for positive lookahead" do
      ...
      first = atom.first_set
      expect(first).to eq(Set.new([Parslet::FirstSet::EPSILON]))
    end
  ...
  describe "Complex grammars" do
    ...
    it "computes FIRST for expression-like pattern" do
      ...
      expect(first.size).to eq(2)
      has_paren = first.any? { |x| x.is_a?(Parslet::Atoms::Str) && x.str == '(' }
      has_digit = first.any? { |x| x.is_a?(Parslet::Atoms::Re) }
  ...
  describe "Parslet::FirstSet class methods" do
    describe ".disjoint?" do
      it "returns true for disjoint sets" do
        set1 = Set.new([str('if')])
        set2 = Set.new([str('while')])
        expect(Parslet::FirstSet.disjoint?(set1, set2)).to be true
      end
      ...
      it "returns false for overlapping sets" do
        ...
        set1 = Set.new([atom])
        set2 = Set.new([atom])
        expect(Parslet::FirstSet.disjoint?(set1, set2)).to be false
      end

      it "ignores EPSILON when checking disjointness" do
        set1 = Set.new([str('a'), Parslet::FirstSet::EPSILON])
        set2 = Set.new([str('b'), Parslet::FirstSet::EPSILON])
      ...
      it "returns true for empty sets" do
        set1 = Set.new([Parslet::FirstSet::EPSILON])
        set2 = Set.new([str('a')])
    ...
    describe ".all_disjoint?" do
      it "returns true for mutually disjoint sets" do
        ...
        expect(Parslet::FirstSet.all_disjoint?(sets)).to be true
      end
      ...
      it "returns false when any two sets overlap" do
        ...
        expect(Parslet::FirstSet.all_disjoint?(sets)).to be false
      end
      ...
      it "handles sets with EPSILON correctly" do
        sets = [
          Set.new([str('a'), Parslet::FirstSet::EPSILON]),
          Set.new([str('b'), Parslet::FirstSet::EPSILON]),
          Set.new([str('c'), Parslet::FirstSet::EPSILON])
        ]
